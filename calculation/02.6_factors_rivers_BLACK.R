...
p1 <- ggplot(d_p2 %>% filter(polymer %in% c("PP", "PS", "PET")), aes(x = L, y = value, color = polymer, linetype = polymer))+
	geom_line()+
	scale_color_manual(values = c("orange", "lightblue", "grey90"))+
...
p1
# ggsave(paste0(main.path, "output_files/plots/plot_factor_river_black.png"), plot = p1, width = 12, height = 8, units = "cm", bg = "transparent", dpi = 500)
ggsave("PhD/präsi/für Konferenzen/2023 SETAC/factor_rivers1.png", plot = p1, width = 13, height = 8, units = "cm", bg = "transparent", dpi = 500)
