...
############################
	...
	load(paste0(main.path, "temp_data/all_burial/print_round.Rdata"))
	rounds <- print_round
	...
	nselection <- c("NAME", "isLake", "id_all", "flow_velocity", "flow_to", "outflow", "name_river", "verweilzeiten")
	...
	for (extra_name in extra_names) {
		...
		for(mat in polymers){
			...
			for (NR in rounds) {
				...
				rivers.calc[ , paste0("accumulation_tot_LAKE_", mat, "_MiP")] <- NA #multiplied with Verweilzeit for interpretation of total amount of plastics in the lake
				rivers.calc[ , paste0("accumulation_perM_LAKE_", mat, "_MiP")] <- NA
				# rivers.calc[ , paste0("removal_tot_", mat, "_MiP")] <- 0 #nicht relevant für MiP
				# rivers.calc[ , paste0("removal_perM_", mat, "_MiP")] <- 0
				rivers.calc[ , paste0("inputKawecki_tot_", mat, "_MiP")] <- rivers.calc[ , paste0("sum_",mat, "_WaterMP")]
				rivers.calc[ , paste0("inputKawecki_perM_", mat, "_MiP")] <- NA
				...
				rivers.calc[r.rivers , paste0("inputKawecki_perM_", mat, "_MiP")] <- rivers.calc[r.rivers, paste0("sum_", mat, "_WaterMP")] / rivers.calc$length_m[r.rivers]
				...
				#for sediments
				rivers.calc[ , paste0("sediment_tot_", mat, "_MiP")] <- rivers.calc[ , paste0("actualsed_", mat, "_WaterMiP_concMSV")] 
				
				rivers.calc[ , paste0("sediment_perM_", mat, "_MiP")] <- rivers.calc[ , paste0("sediment_tot_", mat, "_MiP")] / rivers.calc$length_m
				...
				rivers.calc[r.lakes , paste0("inputKawecki_perM_", mat, "_MiP")] <- rivers.calc[r.lakes, paste0("sum_", mat, "_WaterMP")] / rivers.calc$area[r.lakes]
				...
				for (id in lake_id) {
					...
					rivers.calc[x, paste0("accumulation_tot_LAKE_", mat, "_MiP")] <- rivers.calc$verweilzeiten[x]*rivers.calc[x , paste0("accumulation_tot_", mat, "_MiP")]
					rivers.calc[x, paste0("accumulation_perM_LAKE_", mat, "_MiP")] <- rivers.calc[x , paste0("accumulation_tot_LAKE_", mat, "_MiP")] / rivers.calc$area[x] #area in m2
				...
				save(s_r, file = paste0(main.path, "temp_data/all_burial/cont_",mat,"_", NR, extra_name, ".Rdata"))
				save(s_r2, file = paste0(main.path, "temp_data/all_burial/lakes_single_cont_",mat,"_", NR, extra_name, ".Rdata"))
				Sys.sleep(2)
			}
		}
	}
