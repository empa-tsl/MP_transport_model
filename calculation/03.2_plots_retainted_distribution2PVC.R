...
#plot
p1PVC <- ggplot()+
	geom_area(data = df02 %>% filter(polymer == "PVC"), aes(x = km, y = value, fill = variable), stat = "identity")+
	...
	annotate("text", x = max(df02$km)*0.1, y = 0.2, size = 2, label = "PVC")+
	theme(plot.margin = unit(c(0,0,0.1,0), units = "cm"))
legend <- get_legend(p1PVC)
p1PVC <- p1PVC+theme(legend.position = "none")

p1LDPE <- ggplot()+
	geom_area(data = df02 %>% filter(polymer == "LDPE"), aes(x = km, y = value, fill = variable), stat = "identity")+
	scale_y_continuous(expand = c(0,0), breaks = c(0,0.5,1), labels = c("0", "0.5", "1"))+
	scale_x_continuous(limits = c(0, max(df02$km)), expand = c(0,0))+
	scale_fill_manual("",values = farben,
	...
	annotate("text", x = max(df02$km)*0.1, y = 0.2, size = 2, label = "LDPE")+
...
load(paste0(main.path, "output_files/rdata/cont_PVC_", rounds, without, ".Rdata"))
rm(s_r)
...
# 
# 
# pall <- ggdraw(plot_grid(p1PVC,p1LDPE, p1PVC, p2PVC, p2LDPE, p2PVC, p3PVC, p3LDPE, p3PVC, ncol = 1, align = "v", rel_heights = c(1.25,0.9,1.25,1.2,0.9,1.25,1.2,0.9,1.25)))
# 
# 
...
x1 <- ggdraw(plot_grid(p1PVC,t0, p11, nrow = 1, rel_widths = c(1,0.08, 0.25), align = "h", labels = c("a)","", "d)"), label_fontface = "bold", label_size = 7, label_x = -0.01))
x2 <- ggdraw(plot_grid(p1LDPE,t0, p21, nrow = 1, rel_widths = c(1,0.08, 0.25), align = "h"))
x3 <- ggdraw(plot_grid(p1PVC,t0, p31, nrow = 1, rel_widths = c(1,0.08, 0.25), align = "h"))

x4 <- ggdraw(plot_grid(p2PVC,t0, p12, nrow = 1, rel_widths = c(1,0.08, 0.25), align = "h", labels = c("b)","", "e)"), label_fontface = "bold", label_size = 7, label_x = -0.01))
x5 <- ggdraw(plot_grid(p2LDPE,t0, p22, nrow = 1, rel_widths = c(1,0.08, 0.25), align = "h"))
x6 <- ggdraw(plot_grid(p2PVC,t0, p32, nrow = 1, rel_widths = c(1,0.08, 0.25), align = "h"))

x7 <- ggdraw(plot_grid(p3PVC,t0, p13, nrow = 1, rel_widths = c(1,0.08, 0.25), align = "h", labels = c("c)","", "f)"), label_fontface = "bold", label_size = 7, label_x = -0.01))
x8 <- ggdraw(plot_grid(p3LDPE,t0, p23, nrow = 1, rel_widths = c(1,0.08, 0.25), align = "h"))
x9 <- ggdraw(plot_grid(p3PVC,t0, p33, nrow = 1, rel_widths = c(1,0.08, 0.25), align = "h"))

pall_all <- ggdraw(plot_grid(plot_grid(x1, x4,x7, nrow = 3, align = "v", rel_heights = c(1.25,1.25,1.25)), legends, nrow = 2, rel_heights = c(1,0.2)))

ggsave("PhD/mennekes/output_files/plots/retain_distribution2_PVC.png", plot = pall_all, width = 18, height = 7,units = "cm", dpi = 500, bg = "transparent")

ggsave("PhD/mennekes/output_files/plots/retain_distribution2_PVC.pdf", plot = pall_all, width = 18, height = 7,units = "cm", dpi = 500, bg = "transparent")
