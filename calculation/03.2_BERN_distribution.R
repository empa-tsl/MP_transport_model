...
for (mat in rev(polymers)) {
		load(paste0(main.path, "temp_data/BERN/cont_", mat,"_", rounds, "_withallfactors", ".Rdata"))
		df00_water[ , counter] <- s_r[s, paste0("water_perM_",mat,"_MiP")] # water values in g/s
		df00_acc[ , counter] <- s_r[s, paste0("accumulation_perM_",mat,"_MiP")]
		df00_sed[ , counter] <- s_r[s, paste0("sedimentation_perM_",mat,"_MiP")]#acc values in g/s
		counter <- counter +1
}
