...
###############################
	...
	#define different scenarios in which only certain factors are applied
	#uncommand your option
	extra_name01 <- "withallfactors" #all factors are applied
	extra_name02 <- "lakeonlyfactors" #sedimentation (including burial etc.) is only possible in lakes
	extra_name03 <- "withoutfactors" #no factors are applied
	extra_name04 <- "lake15" #only factors (sedimentatin etc.) for the 15 biggest lakes
	extra_name05 <- "burial100" #burial rate is 100% normally its 10% until max rate
	extra_name06 <- "resusNULL" #no resuspension
	extra_name07 <- "resus100" #100% resuspension
...
#############
	...
	##################
			...
			#generally, rates are a result of removal per second. The rate can be multiplied with the ms contamination value because the rates take into account the length of each section 1-.... for getting the part that will sediment
	...
	save(rivers.all6,extra_name, burial_rate_rivers, file = paste0(main.path, "temp_data/rivers_all6_", extra_name, ".Rdata"))
	
	ra6 <- rivers.all6 #to mess around
	
	## make resus 100%
	extra_name <- extra_name07
	rivers.all6 <- ra6
	rivers.all6$resus.fac_MiP <- 1
	save(rivers.all6,extra_name, burial_rate_rivers, file = paste0(main.path, "temp_data/rivers_all6_", extra_name, ".Rdata"))
	
	
	## make burial 100%
	extra_name <- extra_name05
	rivers.all6 <- ra6
	rivers.all6[ , c.acc_MiP] <- 1
	burial_rate_rivers <- 1
	save(rivers.all6,extra_name, burial_rate_rivers, file = paste0(main.path, "temp_data/rivers_all6_", extra_name, ".Rdata"))
	
	#resuspension rate 0
	extra_name <- extra_name06
	rivers.all6 <- ra6
	rivers.all6$resus.fac_MiP <- 0
	save(rivers.all6,extra_name, burial_rate_rivers, file = paste0(main.path, "temp_data/rivers_all6_", extra_name, ".Rdata"))
	
	#extra names 2 (lakeonly factors)
	extra_name <- extra_name02
	rivers.all6 <- ra6
		rivers.all6[ r.rivers, c.fac] <- 0
		burial_rate_rivers <- 0 
	save(rivers.all6,extra_name, burial_rate_rivers, file = paste0(main.path, "temp_data/rivers_all6_", extra_name, ".Rdata"))
	
	#extra names 4 (lake15)
	extra_name <- extra_name04
	rivers.all6 <- ra6
	rivers.all6[ !(rivers.all6$lake15) , c.fac] <- 0 #for all lines that are not lake 15, factors are 0
	burial_rate_rivers <- 0 
	save(rivers.all6,extra_name, burial_rate_rivers, file = paste0(main.path, "temp_data/rivers_all6_", extra_name, ".Rdata"))
	
		
		# no factors
	extra_name <- extra_name03
	rivers.all6 <- ra6
			rivers.all6[ , c.fac] <- 0
			burial_rate_rivers <- 0
			save(rivers.all6,extra_name, burial_rate_rivers, file = paste0(main.path, "temp_data/rivers_all6_", extra_name, ".Rdata"))
