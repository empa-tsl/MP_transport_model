...
############################
	...
	sub.path <- "temp_data/BERN/"

	
	# define output folder
	output.gpkg <- paste0(main.path, "temp_data/BERN/")
	...
	load(paste0(main.path, "temp_data/BERN/print_round.Rdata"))
	rounds <- print_round
	...
	for (extra_name in extra_names) {
		...
		for(mat in polymers){
			...
			for (NR in rounds) {
				...
				save(s_r, file = paste0(main.path, "temp_data/BERN/cont_",mat,"_", NR, extra_name, ".Rdata"))
				save(s_r2, file = paste0(main.path, "temp_data/BERN/cont_lakes_single_cont_",mat,"_", NR, extra_name, ".Rdata"))
				Sys.sleep(10)
			}
		}
	}
