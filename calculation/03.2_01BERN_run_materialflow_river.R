...
#loop for extra_names
	...
	save(geo, file = paste0(main.path, "temp_data/BERN/0000_geo.Rdata"))
	...
	rivers.calc[ 117290, sss] <- 1 #here inflow 1
	
	
	rivers.calc.all <- rivers.calc
	...
	for (mat in polymers) {
		...
		for (i in 1:N) {
			...
			if(i %in% print_round){
				Sys.sleep(10)
				save(rivers.calc, file = paste0(main.path, "temp_data/BERN/round_", i, "_", mat,"_",  extra_name, ".Rdata"))
...
save(print_round,extra_names, file = paste0(main.path, "temp_data/BERN/print_round.Rdata"))
rm(list = ls())
