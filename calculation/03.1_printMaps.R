...
############################
	...
	for (extra_name in extra_names) {
		...
		for(mat in polymers){
			rfiles_short_mat <- rfiles_short[grepl(paste0("_", mat, extra_name, ".Rdata"), rfiles_short)] #read files
			...
			for (NR in rounds) {
				...
				rivers.calc[ , paste0("water_perS_", mat, "_MiP")] <- 0 #perS == transported per second
				...
				rivers.calc[ , paste0("inputKawecki_perS_", mat, "_MiP")] <- rivers.calc[ , paste0("sum_",mat, "_WaterMP_concMSV")]
				...
				save(s_r, file = paste0(main.path, "output_files/rdata/cont_",mat,"_", NR, extra_name, ".Rdata"))
				save(s_r2, file = paste0(main.path, "output_files/rdata/lakes_single/cont_",mat,"_", NR, extra_name, ".Rdata"))
				Sys.sleep(10)
			}
		}
	}
