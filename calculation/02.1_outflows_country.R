	...
	#load data
	load(paste0(main.path, "data_modified/river_network/rivers_all01.Rdata"))
	...
	outflows_container <- st_read(paste0(main.path, "data_modified/river_network/outflow_lines.shp")) #this data was obtained manually in a GIS Software, country = "none" is container for NAs
	...
	#####
	#outflows =2 and outflow=4 flow to each self.. for controll
	...
	rm(rivers.all2a)
	save(rivers.all2, file = paste0(main.path, "data_modified/river_network/rivers_all2.Rdata"))
	# st_write(rivers.all2, paste0(main.path, "data_modified/river_network/rivers_all2.gpkg"), append = F)
